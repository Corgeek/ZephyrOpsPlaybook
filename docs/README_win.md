...
7z x zephyr-sdk-0.16.8_windows-x86_64.7z -o%SystemDrive%
```

> [!CAUTION]
ユーザー名にスペース文字を含んでいる場合、SDK をユーザーフォルダ以下に配置するとビルド時にエラーが発生します(e.g. ユーザー名：Yamada Taro)。無難な回避策は `-b C:\` でC:ドライブ直下にインストールです。(`C:\Program Files\` でもOKですが管理者権限が必要)
